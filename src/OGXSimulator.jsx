    ...
    steps: [
      {
        ...
        options: [
          o("solar", "Solar panels", "Pick your panel size then add a charge controller. Higher system voltage = fewer controllers.", 290, ["needsCC"], "PROS: Most accessible, panels keep getting cheaper, 25yr lifespan, works everywhere sun shines. CONS: Only generates during daytime, needs battery for night use, large roof space for meaningful power.",
            {budget:{min:200,max:10000,step:100,def:2000},choices:[
              {key:"cond",label:"Condition",opts:[{id:"used",l:"2nd-hand $0.30/W",e:"Used Grade A: removed from residential, ≥90% rated, ~$0.30/W. Example: 250W ~$75"},{id:"new",l:"New $0.40/W",e:"New budget mono PERC, 25yr warranty. Example: Renogy 100W ~$100, Jinko 400W ~$160"}]},
              {key:"type",label:"Cell type",opts:[{id:"mono",l:"Mono PERC",e:"Standard 2026 tech: 21-23% efficiency, black frame, best value"},{id:"topcon",l:"N-Type TOPCon ×1.15",e:"Next-gen: 22-24% efficiency, better low-light, less degradation"},{id:"bifacial",l:"Bifacial ×1.2",e:"Dual-sided glass, +10-20% from ground albedo, best for ground mount"}]},
              {key:"size",label:"Panel wattage",opts:[{id:"100",l:"100W (Renogy)",e:"Small portable: 36-cell, Voc=22V, Vmp=18V, 5.3A. ~$0.80-1.00/W"},{id:"200",l:"200W (Rich Solar)",e:"RV/van favorite: Voc=24V, Vmp=20V, 9.8A. ~$0.70-0.90/W"},{id:"400",l:"400W (Jinko)",e:"Residential standard: 144 half-cell, Voc=50V, Vmp=41V, 9.8A. ~$0.35-0.50/W"},{id:"550",l:"550W (Can. Solar)",e:"Large format bifacial: Voc=49V, Vmp=41V, 13.4A. ~$0.30-0.45/W"}]},
              {key:"cc",label:"Charge controller",opts:[{id:"pwm",l:"PWM 30A +$30",e:"Budget, 12V only, ~20% loss. Pair with ≤400W at 12V. 60A=800W max"},{id:"mppt40",l:"MPPT 40A +$150",e:"Victron SmartSolar 100|40: 12V=520W, 24V=1040W, 48V=2080W max"},{id:"mppt100",l:"MPPT 100A +$350",e:"Victron 150|100: 12V=1300W, 24V=2600W, 48V=5200W. High voltage = fewer controllers"}]},
              {key:"mount",label:"Mount",opts:[{id:"roof",l:"Roof +$0",e:"Simplest, uses existing structure, flush mount z-brackets"},{id:"ground",l:"Ground +$200",e:"Adjustable tilt frame, better airflow, easier cleaning"},{id:"tracker",l:"Tracker +$500",e:"Single-axis sun tracking, +25-35% daily yield. Needs space"}]},
              {key:"cool",label:"Cooling",opts:[{id:"none",l:"Passive +$0",e:"Standard air gap behind panel. Loses ~0.4%/°C above 25°C"},{id:"active",l:"Active vent +$80",e:"Fan cooling, reduces temps 15°C, +6% output in hot climates"}]}
            ],output:(v)=>{const w=v.cond==="new"?0.40:0.30;const tm=v.type==="mono"?1:v.type==="topcon"?1.15:1.2;const pw=parseInt(v.size)||400;const ccc=v.cc==="pwm"?30:v.cc==="mppt40"?150:v.cc==="mppt100"?350:0;const mc=v.mount==="ground"?200:v.mount==="tracker"?500:0;const clc=v.cool==="active"?80:0;const fixedCosts=mc+clc+ccc;const panelBudget=v._budget-fixedCosts;const panelCost=w*tm*pw;const count=panelBudget>0?Math.max(0,Math.floor(panelBudget/panelCost)):0;const panelSpend=count*panelCost;const totalW=count*pw;const ac=count>0?fixedCosts+panelSpend:0;const dailyKwh=totalW>0?Math.round(totalW*3.5/1000*10)/10:0;const ccAmps=v.cc==="pwm"?30:v.cc==="mppt40"?40:v.cc==="mppt100"?100:0;const max12v=ccAmps*12;const max24v=ccAmps*24;return{fields:[[\"Panels\",count>0?count+\"x \"+pw+\"W\":\"Need more budget\"],[\"Array size\",totalW>=1000?(totalW/1000).toFixed(1)+\"kW\":totalW+\"W\"],[\"Daily yield\",dailyKwh+\" kWh/day\"],[\"Monthly\",Math.round(totalW*3.5*30/1000)+\" kWh/mo\"],[\"Controller\",v.cc===\"mppt100\"?\"MPPT 100A\":v.cc===\"mppt40\"?\"MPPT 40A\":v.cc===\"pwm\"?\"PWM 30A\":\"None\"],[\"Max 12V/24V\",max12v+\"W / \"+max24v+\"W\"],[\"$/W\",\"$\"+(w*tm).toFixed(2)]],spent:Math.round(ac),budget:v._budget};}}),
